...
export const enAiMessages: Messages = {
  ...
  'ai.chat.strictPrivacyExternalBlocked': 'Strict local privacy mode blocks external agents. Select MagiesTerminal Agent with a loopback provider, or turn the mode off in Settings → AI.',
  'ai.chat.strictPrivacyEndpointBlocked': 'Strict local privacy mode blocks this provider because its endpoint is not loopback. Select a localhost provider or turn the mode off in Settings → AI.',
  'ai.title': 'AI',
  'ai.description': 'Configure AI providers, agents, and safety settings',
  'ai.localPrivacy.title': 'Local / private models',
  'ai.localPrivacy.badge': 'On-device',
  'ai.localPrivacy.description': 'Run models on this machine with Ollama or LM Studio. Traffic stays on localhost — no cloud API key required for basic setup.',
  'ai.localPrivacy.addOllama': 'Add Ollama',
  'ai.localPrivacy.addLmStudio': 'Add LM Studio',
  'ai.localPrivacy.addedOllama': 'Ollama added',
  'ai.localPrivacy.addedLmStudio': 'LM Studio added',
  'ai.localPrivacy.safetyNote': 'Vault passwords and private keys are never sent to the model. Write tools still require confirm-mode approval.',
  'ai.localPrivacy.autoModeWarning': 'Permission mode is Auto — write tools skip confirmation. Prefer Confirm for local agents.',
  'ai.localPrivacy.useConfirmMode': 'Use Confirm',
  'ai.localPrivacy.strictMode': 'Strict local privacy',
  'ai.localPrivacy.strictModeDescription': 'Only loopback AI endpoints are allowed. Cloud providers, external agents, redirects to remote hosts, and web search are blocked.',
  'ai.localPrivacy.probe': 'Probe tools',
  'ai.localPrivacy.probing': 'Probing…',
  'ai.localPrivacy.selectModel': 'select a default model',
  'ai.localPrivacy.toolsSupported': 'Tool calling supported',
  'ai.localPrivacy.toolsUnsupported': 'Tool calling not detected',
  'ai.localPrivacy.toolsUnsupportedAdvice': 'Agent tools stay unavailable on this model. Pick a build that advertises function calling, or use this model for plain chat only.',
  'ai.localPrivacy.probeFailed': 'Probe failed: {detail}',
  'ai.safety.approvalAudit.title': 'Tool approval audit',
  'ai.safety.approvalAudit.description': 'Recent approve / deny decisions for MagiesTerminal and MCP write tools (metadata only — no arguments or secrets).',
  'ai.safety.approvalAudit.empty': 'No approval events yet.',
  'ai.safety.approvalAudit.clear': 'Clear',
  'ai.safety.approvalAudit.requested': 'Requested',
  'ai.safety.approvalAudit.approved': 'Approved',
  'ai.safety.approvalAudit.denied': 'Denied',
  'ai.safety.approvalAudit.timeout': 'Timed out',
  ...
  'ai.providers.retryLoadModels': 'Retry',
  ...
  'ai.providers.liveModelCount': 'Loaded {count} models from provider',
  'ai.providers.usingPresetModels': 'Using offline preset list (live catalog unavailable)',
  'ai.providers.needApiKeyForModels': 'Enter an API key to load the live model list',
  'ai.providers.fetchModelsFailed': 'Failed to load models: {detail}',
  'ai.providers.emptyModelCatalog': 'Provider returned an empty model list',
  'ai.providers.modelsBridgeUnavailable': 'Model fetch is unavailable in this window. Fully restart MagiesTerminal and try again.',
  ...
  'ai.codex.description': 'Connect ChatGPT coding agent (OpenAI Codex CLI). Sign in with ChatGPT here, or use an OpenAI-compatible API key / custom endpoint.',
  ...
  'ai.codex.customConfigMissingEnvKey': 'Warning: {envKey} is not set in your shell environment. Export it (or launch magiesTerminal from a shell that has it) so ChatGPT (Codex) can authenticate.',
  'ai.codex.notConnected': 'Not connected',
  'ai.codex.statusUnknown': 'Status unknown',
  'ai.codex.path': 'Path:',
  'ai.codex.notFoundHint': 'Could not find codex CLI in PATH. Install OpenAI Codex or specify the executable path below.',
  ...
  'ai.geminiProvider.hint': 'Use Gemini models via MagiesTerminal Agent: add a Google AI provider, save an API key, then pick Gemini in the chat model/provider menu.',
  'ai.grokProvider.hint': 'Use Grok models via MagiesTerminal Agent: add an xAI (Grok) provider, save an API key, then pick Grok in the chat model/provider menu.',
  'ai.chat.modelsLoading': 'Loading models…',
  'ai.chat.modelsLoadFailed': 'Could not refresh models',
  'ai.chat.modelsRetry': 'Retry',
  'ai.chat.modelsFromLive': 'Live catalog',
  'ai.chat.modelsFromPresets': 'Offline presets',
  ...
  'ai.quickMessages.search.placeholder': 'Search quick messages…',
  'ai.quickMessages.noSearchResults': 'No quick messages match your search.',
  ...
  'ai.chat.role.you': 'You',
  'ai.chat.role.assistant': 'Assistant',
  ...
  // Desktop pet (Settings → AI → Pet)
  'ai.pet.title': 'Pet',
  'ai.pet.enable': 'Enable desktop pet',
  'ai.pet.enable.description': 'Show a draggable pet that reflects AI activity. It floats above every app, on any corner of your screen.',
  'ai.pet.image.title': 'Pet image',
  'ai.pet.image.upload': 'Upload image',
  'ai.pet.image.reset': 'Reset to default',
  'ai.pet.image.spriteCols': 'Columns',
  'ai.pet.image.spriteRows': 'Rows',
  'ai.pet.image.spriteHint': 'For a multi-frame sprite sheet, set how many equal-size frames are arranged in the grid. Leave at 1 x 1 for a single static image.',
  'ai.pet.image.invalidFile': 'Please choose a PNG or WebP image.',
  'ai.pet.image.tooLarge': 'Image is too large (max 6 MB).',
  'ai.pet.image.saveFailed': "Couldn't save the image. Try again, or a different image.",
  'ai.pet.hide': 'Hide pet',
  'ai.pet.bubble.running': 'Working on it…',
  'ai.pet.bubble.waiting': 'Waiting for your approval',
  'ai.pet.bubble.done': 'Done!',
  'ai.pet.bubble.failed': 'Something went wrong',
  'ai.pet.customCommand.label': 'Custom command (right-click menu)',
  'ai.pet.customCommand.placeholder': 'e.g. open -a Terminal',
  'ai.pet.customCommand.description': 'Runs when you choose "Run Command" from the pet\'s right-click menu. Executed as typed, without a shell — no pipes, quoting beyond simple "..."/\'...\' grouping, or variable expansion.',
  'ai.pet.hover.idle': 'Idle',
  'ai.pet.hover.hint': 'Click: AI chat · Double-click: main window · Right-click: more',
  'ai.pet.bubble.runningTool': 'Running {tool}…',
  'ai.pet.notification.done.title': 'Task finished',
  'ai.pet.notification.done.body': 'Your AI task wrapped up — click the pet to take a look.',
  'ai.pet.notification.failed.title': 'Task failed',
  'ai.pet.notification.failed.body': 'Something went wrong with your AI task.',
  'ai.pet.appearance.title': 'Appearance',
  'ai.pet.appearance.scale': 'Size',
  'ai.pet.appearance.opacity': 'Opacity',
  'ai.pet.appearance.alwaysOnTop': 'Always on top',
  'ai.pet.appearance.alwaysOnTop.description': 'Keep the pet above other windows.',
  'ai.pet.appearance.showBubble': 'Show status bubble',
  'ai.pet.behavior.title': 'Behavior',
  'ai.pet.behavior.privacyMode': 'Privacy mode',
  'ai.pet.behavior.privacyMode.description': 'Hide the active tool/task name in the bubble — show only a generic status.',
  'ai.pet.behavior.notifications': 'Notify on long tasks',
  'ai.pet.behavior.notifications.description': 'Show a desktop notification when a task running 10s or longer finishes.',
  'ai.pet.frameRanges.title': 'Per-status animation frames',
  'ai.pet.frameRanges.description': 'For a sprite sheet, choose which frames play for each status. Leave as the full range to cycle the whole sheet.',
  'ai.pet.frameRanges.start': 'Start',
  'ai.pet.frameRanges.end': 'End',
  'ai.pet.status.idle': 'Idle',
  'ai.pet.status.running': 'Running',
  'ai.pet.status.waiting': 'Waiting',
  'ai.pet.status.done': 'Done',
  'ai.pet.status.failed': 'Failed',
  'ai.pet.customCommand.testRun': 'Test Run',
  'ai.pet.customCommand.testRun.running': 'Testing…',
  'ai.pet.customCommand.testRun.success': 'Looks good — the command started.',
  'ai.pet.customCommand.testRun.failure': 'Failed: {error}',
};
