# 更新日誌


## [0.6.4] - 2026-09-06

### 修復
- **值裡的空行不再把語句切成兩半**：匯入和整庫匯出都是把多條 INSERT 用空行拼在一起、再依空行拆回單條執行的。備註、描述這類欄位裡只要有一個空行，拆分就會從字串中間切開，兩半都是語法錯誤。現在語句以列表形式傳遞，不再需要還原邊界
- **結果表格換查詢後不再顯示上一次的資料**：表格在兩次查詢之間不會重建，而儲存格編輯的覆蓋值、被刪除的列、正在編輯的位置都存在它內部。於是跑完第二個查詢後，上一次的編輯值會被畫到同樣列號的新資料上——顯示出一個新結果裡根本沒有的值。翻頁尤其容易撞上，因為每一頁都是一次新結果。篩選條件保留，排序隨欄位變化重置
- **資料表設計器切換目標時會重新載入**：設計器只在開啟時讀一次欄位。開著 A 表的設計器再從樹裡選 B 表，面板會變成「B 的表名 + A 的欄位」，此時套用會把 B 的欄位全部刪除、把 A 的欄位全部新增。執行前雖然能看到 SQL，但它是以一次普通編輯的形式給出的
- **分頁不再把引號裡的欄位名當成關鍵字**：判斷語句有沒有自帶排序時會掃描關鍵字，但只遮蔽了字串和註解，沒遮蔽被引號包住的識別字。於是名叫 `offset` 的欄位會讓分頁被拒絕，名叫 `order by` 的欄位會讓 SQL Server 收到一條沒有排序可依附的 OFFSET，欄位名裡的括號還會打亂巢狀層級的判斷
- **查詢建構器依欄位型別決定加不加引號**：此前只要值長得像數字就不加引號。對文字欄位篩選 `123` 會產生 `"code" = 123`，PostgreSQL 直接報錯、MySQL 則隱式轉換給出意外結果；`007` 也永遠比對不到存著 "007" 的列
- **匯入推斷的欄位寬度不再撐爆列長**：MySQL 與 MariaDB 限制單列所有欄位合計 65535 位元組，utf8mb4 每字元算四位元組，此前推斷出的 varchar(4000) 意味著幾個寬欄位就無法共存於一張表。超過一定寬度改用無長度上限的型別
- **升級四個存在已知漏洞的相依套件**：dompurify、undici、hono 此前釘住的版本本身已被揭露漏洞，js-yaml 則未釘版本。四者分別升到 3.4.15、6.28.1、4.13.7、4.3.2，均在各自大版本內，無破壞性變更。至此從生產相依可達的漏洞全部清零

## [0.6.3] - 2026-09-06

### 新增
- **視覺化資料表結構編輯**：新增資料表、修改資料表、增刪改欄位、建立與刪除索引、更名與刪除資料表，都在一個欄位表格裡完成。產生的 SQL 會先顯示出來，確認後才執行。SQLite 無法修改欄位型別（那需要重建資料表），因此它會直接說明，而不是送出一條伺服器必然拒絕的語句
- **資料匯入與匯出**：可匯入 CSV、TSV 與 JSON 並依內容推斷欄位型別，也可直接建立資料表；匯出在 CSV、JSON、INSERT 之外新增 Markdown、XML 與 HTML；整庫匯出會寫出每張資料表的建立語句與資料，並可由檔案回灌還原
- **新增 SQLite 與 MariaDB**：SQLite 走 Node 內建驅動，無需安裝任何東西；它是本機上的一個檔案而非伺服器，因此沒有主機、連接埠和帳號，也不需要通道。MariaDB 與 MySQL 同協定同方言
- **分頁、篩選、指令碼與查詢建構器**：結果按頁取回，不再停在固定列數上；表格可就地篩選列；編輯器執行整段指令碼而非只跑第一條語句；查詢建構器用選擇器拼出單表 SELECT，結果放進編輯器供繼續修改

### 修復
- **資料表名稱終於帶上所屬 schema**：結構樹會列出伺服器上所有 schema 的資料表，但此前只回報名稱本身，欄位、主鍵、索引與外鍵的查詢也只依名稱過濾。兩個 schema 裡存在同名資料表時，它們在樹上合成一條，中繼資料是兩張表混在一起的結果；更糟的是資料列編輯會據此拼出 WHERE，可能用上一個當前資料表根本沒有的欄位
- **二進位、日期與布林值不再被寫成伺服器讀不回的形式**：BLOB 欄位被序列化成 JSON 物件——欄位會欣然接受，資料就此毀掉，同樣的值放進 WHERE 又比對不到任何列；日期被轉成 UTC 並帶上 MySQL 的 DATETIME 會直接拒絕的後綴；布林值一律用 TRUE/FALSE，而 SQL Server 沒有這個關鍵字、Oracle 23c 之前也沒有布林型別。現在依引擎各自的寫法產生
- **改動了 0 列的編輯不再回報成功**：主鍵比對不到任何列的 UPDATE 是正常完成的，表格卻把輸入的值貼上去，看起來已經存下了。影響列數本來就在完成回呼裡，只是沒有被讀取。常見原因是結果已經過期——那一列被刪掉了，或者主鍵被改了
- **儲存格可以設為 NULL，也可以刪除整列**：清空儲存格存進去的是空字串，於是一個可為空的欄位一旦有了值就再也清不掉。現在右鍵或 Ctrl/Cmd+0 可寫入 NULL，右鍵也能刪除整列
- **直連資料庫可以加密了**：不走 SSH 通道的連線會把資料庫密碼和每一列資料明文送出——沒有任何一個引擎提供 TLS 選項，SQL Server 的實作更是把加密寫死為關閉。現在有三檔可選，預設仍是關閉，因此已儲存的連線行為不變
- **跨主機傳輸不再把檔案留在公用暫存目錄**：跨主機複製會先把整個檔案下載到本機中轉，而這個中轉檔被寫進系統公用暫存目錄裡一個可預測的路徑，任何本機使用者都能讀；上傳失敗時它還會一直留在那裡。現在走應用自己權限 0700 的暫存目錄，且失敗時同樣清理
- **直接掛了金鑰的主機會納入憑證健康檢查**：該檢查讀取的是主機上並不存在的一個欄位，因此這類主機從未被檢查過，遇到金鑰解密失敗時看到的是「所有驗證方式均失敗」——正是這個檢查本來要避免的誤導提示

## [0.6.2] - 2026-08-01

### 新增
- **資料庫結構樹**：左側列出資料表、檢視表、預存程序、函式與觸發器；展開一張資料表可以看到它的欄位、索引和外來鍵，也能查看它的建表語法。MySQL 與 Oracle 直接回傳伺服器本身的 DDL，PostgreSQL 與 SQL Server 沒有這種能力，其語法由目錄資訊重建並在開頭註明省略了哪些內容
- **SQL 自動完成**：編輯器會補齊目前連線的資料表與欄位名稱。在 `p.` 之後只補該資料表的欄位，並且能解析 FROM 與 JOIN 上綁定的別名
- **直接編輯查詢結果**：連按兩下儲存格即可改值，改動會寫成針對來源資料表的 UPDATE。只有在查詢確實來自單一資料表、該資料表有主鍵、且結果裡包含主鍵欄位時才允許編輯；不滿足時保持唯讀並說明是三者中的哪一種
- **交易控制**：關閉自動提交後由你手動 Commit 或 Rollback，一條 DELETE 可以先看結果再決定是否成為永久事實。重新開啟自動提交會先回復未提交的變更，控制項上寫明了這一點
- **執行計畫**：Run 旁邊多了一個按鈕，用於查看查詢的執行計畫。它只對 SELECT 提供——SQL Server 是靠執行批次來產出計畫的，PostgreSQL 的 EXPLAIN ANALYZE 會真的執行語句，因此「看一下計畫」絕不會變成一次寫入
- **查詢歷史與收藏**：跑過的語句會依連線記錄下來，可搜尋、可點回編輯器；失敗的語句同樣記錄。收藏的查詢不會被筆數上限擠掉，也不會被「清空歷史」刪除
- **關聯圖與結構比對**：ER 圖依相依深度排布各資料表，自我參照和循環外來鍵都能正確處理。結構比對會讀取另一個已儲存連線並產生對齊腳本，其中只有「新增」類語句可直接執行，刪除、改型別、收緊可為空性一律以註解輸出
- **匯出查詢結果**：可匯出為 CSV、JSON 或 INSERT 語句。CSV 遵循 RFC 4180 並附 UTF-8 BOM，Excel 開啟中文不會亂碼；以 `=`、`+`、`-`、`@` 開頭的值會被中和，避免被 Excel 當公式執行

### 修復
- **直連資料庫終於能連上，選了跳板機也真的會走隧道**：不使用 SSH 隧道的連線在開啟工作區時會立刻回報「找不到主機」，根本發不出一個封包；同一處程式碼還漏掉了把主機 ID 傳給後端，導致指定了跳板機的連線實際上直接撥向資料庫——資料庫只對跳板機開放時是一個費解的失敗，兩條路都通時更糟：連線靜默成功，但走在隧道之外
- **儲存的密碼不再變得讀不回來**：加密成功就被當作可用，從未驗證密文能否解回。macOS 鑰匙圈在應用簽章識別變化後會拒絕解密卻仍允許加密，於是密碼被一直寫成讀不回的密文，直到你在一個確信輸對了的連線上看到密碼錯誤。現在加密後會立刻解回比對，不通過就改用本機保險庫
- **SQL Server 上的交易此前是失效的**：它的連線是一個未設上限的連線池（預設十條），`BEGIN TRANSACTION` 與其後的語句會落在不同連線上，工作根本不在交易裡，`COMMIT` 也沒有東西可提交。現在釘死為單一連線
- **JSON 欄位的值不再被寫成 `[object Object]`**：json/jsonb 欄位取回來是已解析的物件，寫回時被當作字串處理，產生的是一個欄位會欣然接受、同時把資料毀掉的值。現在序列化成 JSON

## [0.6.1] - 2026-08-01

### 修復
- **儲存資料庫連線不再覆寫既有的連線**：已儲存的連線在讀取時需要逐筆解密密碼（一次主行程往返），先前若在解密完成前儲存新連線，新連線會附加到一個空清單上並把已存的整份寫掉，同時進行中的讀取又會判定自己過期而丟棄結果——已儲存的連線就此遺失。現在讀取完成前一律不寫入磁碟
- **資料庫連線失敗不再一直停在「連線中」**：SSH 通道建立失敗時拋出的例外沒有被接住，介面會無限期停在連線中且不給任何提示。現在驗證失敗、通道建不起來、連接埠不通等各種失敗都會顯示具體原因

### 優化
- **AI 核准卡片直接顯示完整 SQL 語句**：AI 請求執行寫入類 SQL 時，語句先前只出現在摺疊區的參數 JSON 裡；現在與 shell 指令一樣顯示在卡片標題列，並使用獨立的資料庫圖示，不再標註成 shell 指令的 `$` 提示符

## [0.6.0] - 2026-07-31

### 安全
- **修復自動更新鏈路的憑證外洩漏洞**：`electron-updater` 先前依賴的版本在跨來源重新導向時會外洩 `Authorization` 與 `PRIVATE-TOKEN` 標頭（GHSA-p2f4-r6v6-j797），已升級修復
- **清理其餘相依套件漏洞**：兩個 CRITICAL 等級問題（命令注入）已消除，正式環境相依套件警告從 15 項降至 12 項

### 功能
- **資料庫能力接入 AI 助理**：內建 AI 現在可以查看你已開啟的資料庫連線並執行唯讀查詢；`INSERT`/`UPDATE`/`DELETE` 與 DDL 同樣支援，但每次執行前都會跳出核准視窗並顯示完整 SQL 語句，觀察者模式下直接拒絕。AI 只能使用你已建立的連線，無法自行發起連線——SSH 與資料庫憑證不會交給它

### 修復
- **SFTP 資料夾傳輸前的目錄掃描不再壓垮連線**：傳輸開始前用於統計位元組數和檔案數的兩次預先掃描，先前會無上限並行發出目錄列舉請求；在又深又廣的目錄上，表現為點擊傳輸後先卡住甚至斷線。現在與傳輸階段共用同一個並行上限
- **重名探測失敗時不再可能覆寫既有檔案**：為衝突檔案尋找 `(copy N)` 名稱時，先前任何探測失敗都會被當作「該名稱可用」，一次網路抖動就可能回傳實際已被占用的名稱。現在只有確認路徑不存在才會使用該名稱，其餘情況回退到帶時間戳記的名稱
- **上傳衝突的「全部套用」不再被靜默降級為單次選擇**：在某些時序下，勾選「全部套用」只對目前這一個檔案生效，之後仍會重複詢問
- **取消傳輸不再被誤報為錯誤**：以大寫開頭的取消訊息（如 `Cancelled by user`）先前會被當作失敗並跳出錯誤提示；反過來，錯誤詳情中若包含 `/var/cancelled-jobs` 這類路徑，先前會被誤判為使用者取消而丟棄錯誤資訊

## [0.5.28] - 2026-07-24

### 功能
- **診斷中心**：設定 → 系統新增「診斷中心」卡片，可將崩潰日誌、連線歷史（已去識別化）與 AI 審批稽核一鍵複製或匯出成 JSON 報告；渲染程序的錯誤邊界與全域未捕捉例外/未處理 Promise 拒絕現在也會寫入崩潰日誌，此前這條鏈路是斷開的
- **MCP / CLI 呼叫日誌**：記錄本機 CLI（magies-terminal-tool-cli）與 MCP 用戶端發出的每一次呼叫，僅保存方法名稱、成功/失敗與耗時，不記錄呼叫參數；日誌保留 30 天並自動清理，同時併入診斷中心的匯出報告

## [0.5.27] - 2026-07-23

### 功能
- **輕量資料庫客戶端**：Vault 新增資料庫連線類型，支援透過已儲存的 SSH 主機通道連線 MySQL、PostgreSQL、SQL Server、Oracle，不需在伺服器上額外開放資料庫連接埠；開啟一個資料庫連線會像終端機會話一樣新開一個標籤頁，內建 SQL 編輯器與查詢結果表格；資料庫密碼複用與主機密碼相同的保險庫加密儲存，不會以純文字形式落地本機儲存

## [0.5.26] - 2026-07-23

### 功能
- **分組右鍵新增主機**：主機樹的分組右鍵選單新增「新建主機」，選取後會直接把新主機的分組設為目前分組，不必再從頂部按鈕新建後手動選擇分組

### 優化
- **桌面寵物待機改為走動**：閒置狀態從原地呼吸動畫換成在懸浮視窗內左右踏步走動，帶腳步起伏與轉身翻轉；預設角色沒有多幀行走精靈圖，效果是純 CSS 變換模擬而成
- **監控即時曲線改為霓虹 HUD 風格**：總覽頁「即時曲線」圖表從平滑折線改成階梯折線，搭配固定深色星空底板、雙層發光線條，以及每條線最新取樣點的雷達脈衝光圈

## [0.5.25] - 2026-07-22

### 功能
- **桌面寵物**：在 設定 → AI → 寵物 中開啟後，一隻可拖曳的懸浮寵物會出現在螢幕任意角落，跟隨 AI 狀態切換動畫——閒置呼吸、執行中跳動、等待你確認時搖晃、完成時揮手、失敗時抖動；寵物本就很少取得系統焦點，因此特別關閉了 Electron 對未聚焦視窗的動畫節流，避免看起來像「當機」
- **點擊直達**：單擊寵物開啟/聚焦 AI 對話面板，並盡量跳到目前忙碌中的那個終端機工作階段標籤；雙擊聚焦主視窗；右鍵彈出選單，可執行你在設定裡配置的一條自訂命令、開啟 AI 設定、重設寵物位置或隱藏寵物；懸停顯示更詳細的狀態氣泡
- **自訂外觀**：可上傳自己的圖片或精靈圖作為寵物，精靈圖支援依狀態（閒置/執行中/等待/完成/失敗）分別指定播放的幀範圍；大小、不透明度、是否永遠置頂、是否顯示氣泡均可調整
- **隱私模式與完成提醒**：隱私模式下氣泡只顯示「執行中」這類通用狀態，不透露目前在跑哪個工具；耗時 10 秒以上的任務完成或失敗時，可選擇跳出系統桌面通知
- **位置持久化與多螢幕支援**：拖曳後的位置會被記住，重新啟動應用程式或重新開啟寵物後自動恢復；螢幕中斷連線或解析度變化導致寵物移出可視範圍時會自動歸位

## [0.5.24] - 2026-07-22

### 修復
- **健康檢查探測器先前從未真正讀取過金鑰檔案**：讀取私鑰的非同步函式漏掉了 await，回傳的是尚未展開的 Promise 而非檔案內容，因此每次都被判定為「不像私鑰」並靜默略過。所有依賴本機金鑰檔案而非內嵌儲存的主機，在健康檢查裡必然認證失敗，即使終端連線完全正常
- **本機無法解密的憑證不再被誤判為認證被拒**：仍是加密佔位符的密碼或私鑰會被直接剝離為空後再探測，伺服器自然會拒絕零憑證的登入；現在會先識別出「已設定憑證但本機解不開」，並提示解鎖保險庫或修復安全儲存
- **主機金鑰尚未受信任時不再誤報為認證失敗**：探測器早已在主機金鑰未知或變更時主動扣留所有認證方式，卻從未把這項事實回傳給面板；現在會顯示專門的「主機金鑰未驗證」狀態，並提示先手動連線一次以確認信任
- **加密金鑰被略過的提示不再仰賴運氣**：先前只有在沒有嘗試過任何認證方式時才會顯示「金鑰已加密且未儲存密碼」，但只要本機有 SSH agent 在執行就必定會先嘗試 agent 認證，導致這則提示幾乎從不出現
- **健康檢查現在會重複使用互動式連線時已儲存的金鑰密碼**：先前該密碼只在一般連線裡生效，健康檢查從不查詢，導致受密碼保護的私鑰在終端裡能用、放進健康檢查卻總是報認證失敗

## [0.5.23] - 2026-07-22

### 修復
- **啟動主題腳本先前從未執行**：它負責在介面繪製前套用已儲存的主題、強調色與語言，但因內嵌寫法被 CSP 直接拒絕執行，導致啟動瞬間會閃出錯誤配色。現已移為獨立檔案，未放寬任何安全策略
- **frame-ancestors 改由回應標頭下發**：該指令寫在 `<meta>` 中會被瀏覽器忽略、實際不生效；現由 app:// 回應標頭與開發伺服器下發，並新增測試防止內嵌腳本再次出現

### 功能
- **Cast 錄製回放**：應用先前能錄製 asciinema cast v2 卻打不開，現支援播放/暫停/進度跳轉/1x·2x·4x 倍速；被中途截斷的錄製會跳過損壞行並提示條數，而非拒絕整個檔案
- **工作階段記錄內搜尋**：記錄檢視器支援 Cmd/Ctrl+F 檢索，與即時終端的搜尋互不干擾
- **Hex 面板每列位元組數**：可在 8 / 16 / 32 間切換，切換後已擷取內容立即重排
- **更新記錄依類別篩選**：安全 / 功能 / 修復 / 優化分類標籤，各顯示條目數

### 優化
- **清理無法生效的宣告**：移除一批定義後從未被呼叫的程式碼，包括一個從不驗證卻聲稱存在的團隊權限、一個設了永不生效的群組欄位，以及一份與主行程實作重複、永遠不會被載入的 WAN 邀請解析模組
- **書籤錨點測試加固**：補齊位元組偏移與行號互轉在 CRLF、越界、精確往返等邊界上的覆蓋

## [0.5.22] - 2026-07-21

### 安全
- **團隊稽核簽章真實驗證**：先前只要事件帶 `sig` 欄位就顯示勾號，從不驗證；現在逐筆 HMAC 驗簽，並區分「已驗證 / 遭竄改 / 未簽章 / 無金鑰可驗」四種狀態
- **探測不再回傳憑證**：Proxy 連線測試僅回傳錯誤碼而非原始訊息（HTTP CONNECT 會回顯 `Proxy-Authorization`，ProxyCommand 會回顯命令列）；資料來源欄位對應無法繞過清單的密鑰檢查

### 功能
- **SFTP 批次重新命名**：支援 `{name}` / `{ext}` / `{n}` 樣板與序號補零，執行前算出整批計畫並逐筆預覽；任一處重名或撞上未參與重新命名的檔案即整批拒絕
- **主機欄位批次編輯**：一次修改選取主機的使用者名稱、群組、連接埠與標籤；標籤為附加而非取代，留空表示維持原值；資料來源託管的主機會被排除並提示數量
- **結構化主機搜尋**：支援 `tag:` `user:` `group:` `host:` 篩選與自由文字混用；僅在冒號前為已知欄位名稱時才視為篩選器，IPv6 位址與含全形冒號的中文標籤不受影響
- **F1 快速鍵速查表**：可搜尋、依分類分組，讀取目前實際生效的按鍵綁定；本身也是一條可改鍵的一般快速鍵
- **序列埠十六進位傳送**：按原樣寫入裝置，繞過字元集編碼層，附位元組預覽與 ASCII 對照
- **資料來源欄位對應**：可將 `name` / `ip` / `ssh_port` 之類的欄位名稱對應到規範欄位，供無法修改的外部清單使用
- **接通既有能力**：tmux 向視窗傳送指令、Proxy 連線測試、匯入金鑰時一併辨識憑證、PKCS#11 模組路徑預設

### 修復
- **團隊 Vault 匯入不再假成功**：先前匯入分享包會提示「已匯入 N 台主機」，但主機被丟棄、清單毫無變化；現在真正寫入 Vault 並回報實際落地數量，且只增不覆蓋本機變更與憑證
- **WAN 邀請碼可以加入了**：`magies-follow:2:` 邀請一律被當作 LAN 解析並回報 `version` 錯誤，儘管介面明寫支援；現在依邀請版本分流傳輸層
- **主機匯出遵守勾選**：勾選部分主機後匯出仍會匯出全部
- **下拉選單長標籤不再溢出**：固定寬度導致最長的項目文字跑出選單、圖示被擠成零寬；排序選單在法文下有三個選項溢出
- **tmux 傳送指令可用**：該操作要求一個介面無法提供的窗格序號，導致功能存在卻無法觸發

### 優化
- **SFTP 傳輸顯示剩餘時間**：速率未知時不佔位；目錄列不顯示（其總量以檔案數計）
- **頂列顯示同步新鮮度**：停留即可看到資料上次真正落地的時間，無需展開面板
- **AI 對話匯出保留思考過程**：Markdown 摺疊呈現，純文字縮排呈現，並附耗時
- **AI 設定體驗**：快捷訊息支援搜尋；本機模型未偵測到工具呼叫時說明其影響
- **當機回報可見傳送量**：僅在上傳成功後計數，失敗與去重不計入
- **行程與 tmux 操作改用應用內確認框**：不再使用系統原生彈窗，跟隨應用主題
- **行動端納入品質關卡**：`mobile/` 先前被 lint 與測試完全排除，其測試從未被 CI 執行過

## [0.5.21] - 2026-07-21

### 安全
- **主機上下文深層脫敏**：遞迴移除巢狀物件與陣列中的密碼、Telnet 密碼、私鑰和口令，阻止敏感欄位進入 Agent 上下文
- **公共能力會話範圍**：所有 public capability 統一執行 fail-closed 會話校驗，不能跨越目前聊天暴露的終端會話
- **敏感讀取與審批稽核**：Pod describe 改為 Sensitive Read；審批中繼資料由主程序原子持久化，不記錄工具參數或憑據

### 功能
- **嚴格本地隱私與模型探測**：嚴格模式僅允許回環模型服務，停用外部 Agent/網頁搜尋，並可預檢本地模型工具呼叫能力
- **Docker Compose 管理**：System Manager 新增 Compose 專案與服務檢視，以及需明確操作的 up、restart、down
- **Kubernetes 維運擴展**：結構化 Events、Rollout 狀態/歷史/重啟、非互動 Agent Exec、互動終端 Exec 與回環 Port Forward

### 修正
- **Kubernetes JSON 列表解析**：Namespaces、Pods、Deployments 不再解析易變表格文字，kubectl 錯誤會直接顯示
- **跨平台發佈包**：修正免費無 Developer ID 的 Apple Silicon Fuse 後完整性閃退；Android 統一 JDK 21、SDK/Build Tools 36 與 v0.5.21 中繼資料

## [0.5.20] - 2026-07-21

### 功能
- **System Manager Kubernetes**：遠端 kubectl 管理 Pods/Deployments（列表、日誌、describe）；刪除 Pod / 伸縮 Deployment 需確認；MCP/CLI 同步暴露唯讀與受控寫工具
- **本地 LLM 隱私加固**：Ollama/LM Studio 本地提供商路徑與審批稽核；密鑰不進入 LLM 上下文
- **會話 Cast 錄製增強**：asciinema cast v2 記錄輸入事件；終端 resize 時寫入幾何標記並在啟動時記錄 cols/rows
- **SFTP 衝突比較與真位元組續傳**：衝突對話框展示元資料對比；拖放上傳 resume 走 `startOffset` 追加，不再誤刪目標
- **能力目錄擴展**：Kubernetes 域 catalog 註冊、準外掛 registration、CLI/MCP/側欄 tool specs 對齊

### 優化
- **System Overview 監控觀感**：資源條與概覽頁更貼近即時監控 HUD
- **SSH config / Vault 匯入 UX**：匯入流程與提示文案完善

## [0.5.19] - 2026-07-20

### 優化
- **模型列表自動拉取與 UX**：提供商填入 API Key 後自動載入線上模型目錄；載入中/失敗提示與重試刷新；線上列表優先，離線預設兜底
- **GPT-5.6 / Grok / Gemini 模型預設**：Codex/Cursor/OpenCode 與 OpenAI/xAI/Google 提供商預設模型更新
- **xAI (Grok) 提供商**：新增 `api.x.ai` 快速接入
- **ChatGPT 品牌**：原 Codex 代理顯示名與圖示改為 ChatGPT（OpenAI 標識）；各 Agent 圖示按品牌解析

## [0.5.18] - 2026-07-20

### 優化
- **AI 介面 Claude 排版風格**：對話區置中閱讀欄（約 44rem）、更大行距與字級；使用者訊息為柔和氣泡、助手為無邊框正文；極簡頂欄與圓角作曲家輸入區；傳送鍵改為前景/背景圓鈕；空狀態、最近會話、思考塊與工具組同步弱化裝飾

## [0.5.17] - 2026-07-20

### 優化
- **功能圖示按鈕佈局重設計**：Vault 頂欄工具圖示收入主題化控件簇；搜尋/次要操作/新建主機統一高度與間距；多選條與頂欄 AI/主題/設定工具區同樣分組並跟隨主題色

## [0.5.16] - 2026-07-20

### 優化
- **多色主題更易選**：核心主題明確支援 **Claude 橙 / White 白 / Black 黑** 與藍綠紫玫瑰琥珀天空等多色；外觀設定增加一鍵色條快選

## [0.5.15] - 2026-07-20

### 優化
- **預設 Claude 橙色主題**：暗色/亮色預設 UI 主題改為醒目的 Anthropic/Claude 風格暖橙；核心主題列表首位為 Claude，仍可切換 **Pure Black** 等黑色主題；跟隨應用的終端主題同步匹配

## [0.5.14] - 2026-07-20

### 優化
- **AI 圖示與元件體系重設計**：Agent/Provider 圖示徽章改漸層色盤；工具呼叫卡依類型顯示圖示盤；審批條與狀態 chip 強化；Vault/終端產物卡統一卡片樣式；斜線指令選擇器與匯出選單同步圖示化

## [0.5.13] - 2026-07-20

### 優化
- **AI 側欄全面重設計**：面板環境光與微網格背景；頂欄玻璃化 + 主色「新對話」按鈕；使用者訊息主色實心氣泡、助手訊息卡片帶主色左邊線；底部輸入區懸浮作曲家樣式（聚焦光暈）；空狀態/最近工作階段/工具呼叫組/思考塊/Agent 選單同步升級

## [0.5.12] - 2026-07-20

### 優化
- **一眼可見的 UI 重設計**：Vault 側欄選中項改為主色實心膠囊；主機網格卡片增加左側主色條、懸停抬升與光暈；工作台/側欄對比度加強；設定頁導覽同樣使用主色實心選中；頂欄 Vault/SFTP 增加主色底線；分區標題改為帶圖示的小號大寫樣式

## [0.5.11] - 2026-07-20

### 優化
- **用戶端 UI 主題化視覺重構**：全域殼層、Vault 側欄/工作台、設定頁與共用 UI 原語統一為更貼合主題色的層次與質感——主色導覽高亮、玻璃浮層、柔和陰影與漸層表面，按鈕/輸入/彈窗/Tabs/Switch 等控件焦點與深度更清晰，空狀態與側欄更有視覺焦點

## [0.5.10] - 2026-07-19

### 修復
- **「更新內容」對話框遮罩過暗**：對話框開啟時遮罩過重,將設定視窗左側導覽列壓暗到幾乎看不清;現降低該對話框遮罩的不透明度與模糊,後方導覽保持可讀（僅影響此對話框）

## [0.5.9] - 2026-07-19

### 修復
- **對話框無法關閉**：設定視窗中的對話框（如「更新內容」）因右上角關閉按鈕（X）落在標題列拖曳區、點擊被當作拖曳視窗而無法關閉；現所有對話框均排除於拖曳區之外，X 可正常關閉

## [0.5.8] - 2026-07-19

### 安全
- **本機檔案 IPC 強化**：localFsBridge 的本機檔案讀/寫/刪/列舉介面新增呼叫方 sender 驗證，拒絕 webview/訪客內容，避免轉譯層 XSS 升級為任意本機檔案存取（縱深防禦）
- **相依套件安全強化**：清除生產相依樹全部 high 公告：fast-uri→4.1.1、fast-xml-parser→5.10.1、fast-xml-builder→1.3.0、hono→4.12.31；並將 @cursor/sdk 子樹的 node-gyp→11.4.2、tar→7.5.20（範圍收斂，不影響原生建置）

## [0.5.7] - 2026-07-18

### 功能
- **匿名當機回報(可選)**:預設關閉;在「設定 → 系統」開啟後,向 MagiesTerminal 團隊傳送去識別化的當機摘要(不含路徑、使用者名稱、主機名稱或工作階段資料),協助更快修復當機問題

## [0.5.6] - 2026-07-18

### 安全
- **HTTP inventory 認證標頭加密儲存**：json_http 資料來源的認證標頭(Authorization / API Key)不再以明文寫入本機儲存,改為隨 vault 進行欄位級加密;升級後首次啟動自動遷移既有的明文值
- **相依套件安全強化**：undici 升至 6.27.0、DOMPurify 升至 3.4.12、uuid 升至 13.0.2,修復過時 override 仍命中的可達 XSS 與請求走私 / DoS 公告

## [0.5.5] - 2026-07-18

### 修復
- **自動更新誤報「更新失敗」**：檢查階段錯誤不再視為下載失敗；檢查結束後正確清理 in-flight 狀態
- **Windows arm64 更新頻道**：讀取 `latest-arm64.yml`，避免誤下 x64 安裝包
- **更新檢查與下載路徑更穩**：雙源 feed 與 UI 狀態機對齊，減少併發檢查導致的假錯誤

## [0.5.4] - 2026-07-18

### 安全
- **Vault 解鎖邊界**：停用/改 PIN/註冊或清除 WebAuthn 需已解鎖或驗證目前 PIN；PIN 錯誤限速
- **SSH 診斷/健康檢查**：主機金鑰 unknown/changed 時在認證前中止，避免向中間人傳送密碼
- **工作階段跟隨**：LAN/WAN 應用幀用邀請 token 做 AES-GCM 端到端密封；relay 透明轉發；拒絕偽 wss/ws TLS
- **憑證 IPC**：vault 解鎖與加解密介面校驗呼叫端 sender
- **暫存目錄 / RDP / 深鏈 / 日誌 / AI 附件**：0700 與 symlink 防護、RDP 失敗立即清理 cmdkey、Telnet/JMS 確認、不再記錄 kbd-int 回應、附件大小上限

### 修復
- 健康檢查支援 keyboard-interactive 密碼探測；更新內容彈窗可捲動
- AI 可僅憑附件傳送；SFTP/連接埠轉發可正確傳入 verifyHostKeys

### 工程
- 新增 `npm run typecheck`；修復一批 vault/WebAuthn/更新/SFTP 相關生產型別錯誤

## [0.5.3] - 2026-07-18

### 修復
- **更新內容彈窗無法捲動**：長版釋出說明超出視窗後可正常下拉檢視

### 優化
- 更新日誌彈窗最新版本計數文案修正；10 種介面語言補齊「更新內容」相關文案

## [0.5.2] - 2026-07-18

### 功能
- **本機優先團隊 Vault**：僅中繼資料主機清單包分享、角色（owner/editor/viewer）與 HMAC 簽名稽核；密碼與私鑰永不離開本機
- **工作階段跟隨 WAN 中繼**：TCP NDJSON 中繼支援 NAT 後協作觀看；可內嵌本機中繼或自建 `scripts/follow-relay.cjs`
- **裝置 Passkey 解鎖 Vault**：WebAuthn 平台驗證器（Touch ID / Windows Hello / 安全金鑰）主行程驗證；非雲端多裝置同步
- **內建 ssh2 混合後量子 KEX**：優先 `mlkem768x25519-sha256`，伺服器不支援時回退經典演算法
- **RDP 主機支援**：Vault 一鍵啟動系統遠端桌面用戶端（Windows mstsc、macOS Windows App、Linux xfreerdp）
- **系統 OpenSSH 跳板與代理**：跳板鏈與 HTTP/SOCKS 代理可用於系統 OpenSSH 工作階段

### 優化
- **全域 UI 元件升級**：按鈕、輸入、浮層、側欄、空狀態、Toast 等統一圓角/陰影/焦點環
- **AI 側欄體驗**：問答版面、模型/權限控制、思考指示器（方塊旋轉）與輸入排版
- **更新日誌對話框重設計**：依版本摺疊、分類著色、跟隨介面語言

## [0.5.0] - 2026-07-17

### 功能
- **終端 Hex/Raw 串流診斷面板**：可選開啟，逐位元組檢視工作階段原始輸入輸出，便於排查編碼/跳脫序列問題
- **JSON 主機資料來源**：從本機 JSON 檔案或 HTTP(S) 介面拉取主機清單（CMDB / Ansible / 自訂 API 風格），僅中繼資料、拒絕含金鑰清單；支援 HTTP 認證標頭
- **主機清單分享與匯入**：匯出僅含中繼資料的清單用於團隊交接（含 Ansible YAML 格式），剪貼簿匯入
- **命名工作區範本**：把主機繫結、分割版面、可選 cwd/啟動指令存成範本，快速切換器一鍵套用
- **連線日誌書籤**：重播位置書籤 + 備註 + 搜尋跳轉，日誌清單顯示書籤數
- **連接埠轉送即時通道檢視**：本機/遠端/動態轉送的逐連線來源、目標與流量位元組統計
- **指令碼 onOutput 觸發動作擴充**：命中輸出模式可選桌面通知、提示音、標記分頁、開始工作階段錄製
- **安全貼上與精確廣播**：多行貼上延遲/等待提示符/危險指令確認；廣播可精確指定工作區/選定/分組/視窗
- **系統 OpenSSH 通道增強**：GSSAPI/Kerberos 與後量子（PQ）演算法經系統 OpenSSH 支援；支援跳板鏈與 HTTP/SOCKS 代理
- **內建 ssh2 混合後量子 KEX**：優先協商 `mlkem768x25519-sha256`（ML-KEM-768 + X25519），伺服器不支援時回退經典演算法；不必強制使用系統 ssh
- **RDP 主機支援**：Vault 主機可啟用 RDP，一鍵啟動系統遠端桌面用戶端（Windows mstsc、macOS Windows App、Linux xfreerdp）
- **更新日誌跟隨介面語言**：應用內 Changelog 依目前 UI 語言顯示（10 種語言）

### Windows ARM64
- **win-arm64 安裝包補齊 mosh / ET 綑綁**：MoshMagies 0.1.9 與 EternalTerminal 6.2.10 首發 Windows arm64 原生二進位
- **win-arm64 獨立自動更新來源**：更新中繼資料改走 `latest-arm64.yml` 專用頻道，不再跟隨 x64 更新（此前 arm64 更新會裝到 x64 套件、依賴模擬執行）

## [0.4.10] - 2026-07-17

### 功能
- **SSH 連線診斷中心**：主機編輯面板「測試連線」+ 連線失敗時「執行診斷」，分步檢查 DNS / TCP / 跳板機 / 主機金鑰 / 認證 / SFTP
- **SSH Agent 一等公民認證**：主機可明確選擇 Agent 認證、檢視 Agent 中金鑰指紋並指定偏好身分；連線日誌記錄實際認證方式
- **多主機健康快照**：Vault 一鍵批次檢查線上延遲、認證與負載/記憶體/磁碟，篩選異常主機並執行指令碼
- **SFTP 可靠性一期**：斷點續傳、失敗自動退避重試、傳輸佇列持久化（重啟可恢復）、可選 SHA-256 校驗
- **產品化引導**：首次空 Vault 三步引導；快速切換器指令項（設定/匯入/健康檢查等）；空狀態遷移提示；首次連線成功建議；README 功能矩陣

### 修復
- 升級既有 Vault 的使用者不再彈出首次引導；健康檢查認證失敗時正確關閉跳板連線

## [0.4.9] - 2026-07-17

### 最佳化
- **發布與自動更新來源遷移至獨立發布儲存庫**：安裝包與更新中繼資料現發布在 MgTerminal-releases 儲存庫，官網下載與應用內自動更新體驗不變；既有舊版本用戶端經原網址跳轉繼續正常接收更新

## [0.4.8] - 2026-07-16

### 功能
- **快速連線支援 EternalTerminal**：QuickConnect 精靈新增 ET 協定入口（SSH 連接埠 + ET 服務連接埠，預設 2022），配套 ET 用戶端二進位已隨包綑綁（macOS / Linux / Windows x64）
- **憑證自檢**：設定 → 系統 → 憑證保護新增「自檢」——加解密迴環探測 + 掃描憑證庫，列出在本裝置無法解密的具體條目（主機 / 金鑰 / 身分 / 群組 / 代理），便於鑰匙圈故障後定位需重新輸入的憑證
- **Windows ARM64 安裝包首發**：新增 win-arm64 建置（暫不綑綁 mosh / et，自動更新暫隨 x64 更新來源）
- **工作階段恢復過期清理**：超過 14 天的恢復版面在啟動時自動丟棄，不再恢復大量陳舊佔位符

### 修復
- **俄語介面補齊 203 條缺失文案**（指令碼 / 自動化 / 錄製整個命名空間此前回退英文）、簡體中文補 3 條；新增全量對齊測試防止再回歸
- 快速連線 Mosh 的自訂 mosh-server 路徑此前只收集不生效，現正確寫入主機設定

### 最佳化
- SFTP 全選（Cmd/Ctrl+A）與清單渲染的可見性規則統一為單一實作，消除隱藏檔案 / 過濾詞情境下的行為漂移
- README 的 macOS 說明與實際發布方式對齊（未簽章，附 Gatekeeper 放行步驟；應用內更新不受影響）

## [0.4.7] - 2026-07-15

### 功能
- **介面語言擴充到 10 種**：用戶端與官網對齊，新增日本語 / 한국어 / Deutsch / Français / Español / Português（原有 en / ru / zh-CN / zh-TW 保留）
- 設定 → 外觀 → 語言 可選全部支援語言；未覆蓋的文案仍回退到英語

## [0.4.6] - 2026-07-15

### 安全
- **SSH 主機金鑰校驗關閉時不再靜默**：`verifyHostKeys` 關閉時（終端工作階段與 mosh 統計連線）會記錄明確的告警日誌，說明正在不加詢問地接受任意主機金鑰
- **設定頁持續警示**：關閉「校驗 SSH 主機金鑰」後，開關下方持續顯示中間人風險提示（en / zh-CN / zh-TW）。預設仍為開啟

## [0.4.5] - 2026-07-15

### 修復
- **巢狀密文導致的 401 / 空串流**：鑰匙圈失效期間反覆儲存會把金鑰層層加密（`enc:v2(enc:v1(...))`）；解密迴圈邊界修正後，預算內的多層巢狀能完整解出，不再「解對了又丟棄」或誤報解密失敗
- **單個壞憑證不再拖垮整個憑證庫載入**：欄位解密失敗時改為保留原儲存值（fail-soft），憑證庫照常載入，金鑰在鑰匙圈修復後仍可恢復
- **Web 搜尋 API Key**：解密失敗後僅聚焦/失焦不再誤刪已存金鑰；補充解密/加密失敗的明確提示，不再靜默
- **Windows DPAPI 密文識別修正**：防重複加密守衛此前漏判 DPAPI 金鑰（`AQAAAN` 標頭），鑰匙圈失效時會二次加密成巢狀密文，現已修正
- **Cursor Agent**：解密失敗時不再把密文當 API Key 注入子程序
- 設定頁 Provider / Web 搜尋 / Cursor 三處統一：解密失敗時明確提示重新輸入 Key，切換介面語言不再覆蓋未儲存的 Key

## [0.4.4] - 2026-07-14

### 修復
- **AI 401 / 空串流**：API Key 解密失敗或未同步到主程序時，不再帶著 `__IPC_SECURED__` 佔位符請求供應商；改為立即報錯並提示重新儲存 Key
- 傳送訊息前等待 providers 同步到主程序，避免競態導致鑑權失敗
- 本機 Key 不可用時給出明確 auth 提示（解密失敗 / 缺失 / 佔位符殘留）

## [0.4.3] - 2026-07-14

### 修復
- **API Key 解密**：主程序正確解密 `enc:v2` 本機保險箱金鑰；解密失敗時不再把密文當明文發給供應商（避免 401 與 `…5Q==` 尾綴）
- **憑證佔位符識別**：連線邊界 / 雲同步守衛同步識別 `enc:v2`，避免把本機保險箱密文當密碼發出或上傳到同步
- 模型空串流（`NoOutputGeneratedError`）與 401 鑑權失敗給出可操作的錯誤提示
- Cursor SDK 安裝探測改為 `require.resolve`，避免誤報未安裝

## [0.4.2] - 2026-07-14

### 修復
- **一次性解決 API Key 加密失敗**：鑰匙圈（safeStorage）不可用時自動改用本機加密保險箱（`enc:v2`），應用更新後不再因 Keychain ACL 失效而無法儲存 API Key
- macOS 仍會優先嘗試系統鑰匙圈，失敗時靜默回退；設定 → 系統 可檢視目前後端

## [0.4.1] - 2026-07-14

### 最佳化
- 主題選擇器：卡片預覽（背景 + 主色/次色）、Core / 全部範圍切換、搜尋與空狀態
- 預設 Snow / Midnight 提升對比度與卡片層次，同步 `ui-snow` / `ui-midnight` 終端配色
- 統一選中態與介面層次：Vault 主機/樹、SFTP 清單/樹/分頁列、設定頁導覽、AI 側欄、終端頂列
- 終端主題清單（彈窗 / 側欄）支援搜尋與更清晰色塊預覽
- 將同步狀態、Toast info、更新徽章、拖放高亮等硬編碼色收斂到主題 token

## [0.4.0] - 2026-07-13

### 功能
- 中國使用者下載與自動更新加速：自動識別地區並切換到當地鏡像來源，與 GitHub 雙向回退
- 設定頁「更新內容」改為應用內彈框展示各版本更新記錄，不再跳轉 GitHub
- 新增「問題諮詢」入口，點擊複製聯絡信箱
- SSH 斷線自動重連改為指數退避（5 秒起、最長 60 秒），連續失敗 10 次自動停止並提示手動重連
- 本機/動態連接埠轉送自動重用已認證的終端 SSH 連線，免二次密碼/2FA 驗證
- 匯入 FIDO2 安全金鑰（sk-*）時提示改用 ssh-agent 認證

### 變更
- 設定頁移除「回饋問題」「社群」兩個 GitHub 入口

## [0.3.0] - 2026-07-13

### 修復
- AI 供應商儲存時 API Key 加密失敗不再被靜默吞掉，會在 API Key 下方顯示明確的本地化錯誤提示

## [0.2.9] - 2026-07-13

### 功能
- macOS 支援自動更新：下載後以 bundle 替換方式安裝，繞開未簽章應用的 Squirrel 限制（0.2.9 起全平台可自動升級）

### 修復
- 應用圖示保留官方素材的圓角底板，深淺色下顯示一致

## [0.2.8] - 2026-07-13

### 修復
- Windows 套件啟動即靜默退出：afterPack 重寫 asar 後重新嵌入完整性雜湊，並增加 CI 校驗防止復發
- 更新安裝的進度與錯誤資訊在各平台均可見

## [0.2.7] - 2026-07-13

### 修復
- Windows 發布架構安全的 x64 安裝包

## [0.2.6] - 2026-07-12

### 安全
- 打包版系統匣視窗忽略 `VITE_DEV_SERVER_URL`，並攔截導覽 / 新視窗
- preload 在 `app.asar` 環境下不再把開發伺服器加入可信來源
- 覆蓋升級 DOMPurify 3.3.2、undici 6.23.0，修復可達 XSS / 解壓鏈 DoS
- afterPack 修復 ASAR 檔案完整性雜湊並同步 Info.plist，避免 macOS 啟動即崩潰

### 修復
- Telnet 自動登入整合測試改為等待指令提示符後再斷言完成事件

## [0.2.5] - 2026-07-12

### 修復
- 設定頁社群隱藏「GitHub 原始碼」入口
- 更新內容 / 問題回饋連結改為 `JasonZhangDad/MgTerminal`，修復 404
- 修復「立即重啟」無回應：更新安裝退出不再被 before-quit 非同步髒檢查取消
- 「重啟並更新」失敗時給出明確提示；不支援自動安裝的平台自動開啟 Releases

## [0.2.4] - 2026-07-12

### 安全
- 憑證加密不可用時停止儲存，禁止回退為明文
- SSH 深層連結預設關閉，拒絕包含密碼的 URL，連線前必須確認
- OSC52 剪貼簿預設關閉
- 收緊 Electron CSP，啟用 ASAR 完整性及安全 fuses
- 移除 macOS disable-library-validation 權限

## [0.2.3] - 2026-07-11

### 修復
- 修復打包版 `app://` 主機名被 Chromium 正規化為小寫後，preload 拒絕注入 Electron bridge，導致終端、SFTP、設定、檔案選擇和連接埠轉送等功能不可用
- 統一主視窗、設定視窗及應用權限檢查對 `app://magiesterminal` 的識別，恢復剪貼簿和本機字型權限

## [0.2.2] - 2026-07-11

### 修復
- 主機詳情「Select Color Theme」巢狀 ScrollArea 導致主題點擊無回應；改為單層捲動並用 pointerdown 選擇
- SSH 金鑰/本機金鑰檔案選擇對話框未繫結父視窗，macOS 上無法彈出選檔案
- Settings 視窗在 `app://` 協定下無法開啟
- 側邊欄與安裝包應用圖示改為新圖示資源

## [0.2.1] - 2026-07-11

### CI/CD
- 重新啟用 macOS 和 Windows 自動建置機制（無程式碼簽章模式），提供更多平台的開箱即用套件。

## [0.2.0] - 2026-07-11

### 功能
- 修復自動更新 IPC 事件僅傳送到單一視窗的問題，改為廣播所有視窗（主視窗 + 設定視窗均可收到）
- 統一手動檢查更新與自動更新的狀態機，消除三套並行狀態
- 手動「檢查更新」透過 GitHub API 偵測版本，發現更新後非同步觸發 electron-updater 下載
- 設定視窗中點擊「檢查更新」後，下載進度可即時反映在 UI 中
- 應用啟動後 5 秒自動觸發 `electron-updater` 檢查更新，無需使用者手動點擊
- 發現新版本後自動開始下載（`autoDownload=true`）
- 下載完成後彈出持久 toast 通知，使用者點擊「立即重啟」即可安裝
- 下載失敗時彈出錯誤 toast，提供「開啟 Releases」降級入口
- Settings > System 進度條即時展示自動下載進度，由 `useUpdateCheck` 統一驅動
- Linux deb/rpm/snap 等不支援 electron-updater 的平台自動跳過，保持原有 GitHub API 通知行為

### 設計原理
- `broadcastToAllWindows` 替換 `getSenderWindow` 單點傳送，保證所有視窗都能收到 IPC 事件
- `manualCheckStatus` 欄位追蹤手動檢查 UI 狀態（idle/checking/available/up-to-date/error），與 `autoDownloadStatus` 在 UI 層按優先級渲染
- `SettingsSystemTab` 不再持有本機 update state，單向接收 `useUpdateCheck` 統一資料
- 將原有兩套獨立系統（GitHub API 通知 + electron-updater 手動下載）合併為統一狀態機：`useUpdateCheck` 作為唯一事實來源，同時驅動 `App.tsx` toast 和 `SettingsSystemTab` 進度條
- 全域持久化 IPC 監聽器在 `autoUpdateBridge.init()` 時一次性註冊，避免每次手動下載請求重複註冊/清理監聽器
- `autoInstallOnAppQuit=false`，不做靜默安裝，由使用者主動觸發重啟

### 介面變更（SettingsSystemTabProps）
- 移除：`autoDownloadStatus`、`downloadPercent`
- 新增：`updateState`（完整 UpdateState）、`checkNow`、`installUpdate`、`openReleasePage`

### 注意事項
- `checkNow` 語義：使用 GitHub API（`performCheck`）偵測是否有新版本，若發現更新且 electron-updater 尚未開始下載，則非同步觸發 `bridge.checkForUpdate()` 啟動自動下載流程
- 此功能僅對打包後的應用（Windows NSIS、macOS dmg/zip、Linux AppImage）生效，dev 模式需配合 `forceDevUpdateConfig=true` + `dev-app-update.yml` 測試（見 `.gitignore`）
- `hasUpdate` 舊 toast 在 `autoDownloadStatus !== 'idle'` 時自動抑制，避免與新 toast 重複

### CI / 建置改進
- 跳過 macOS / Windows 建置（需要付費程式碼簽章憑證），專注提供免費 Linux 發行套件
- Linux x64（AlmaLinux 8）編譯器升級：優先使用 Clang，回退 gcc-toolset-13
- Linux arm64（Debian Bullseye）編譯器升級：從 `build-essential` 升級為 `clang-14 + lld-14`
- Release job 不再依賴 macOS/Windows 建置，tag 推送後直接基於 Linux 產物發布 Release
- 軟化 deb 產物校驗：找不到檔案時輸出 warning 而非 error，避免因平台跳過導致 CI 失敗
