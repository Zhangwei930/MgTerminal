...
export const zhCNCoreMessages: Messages = {
  ...
  'settings.system.vaultUnlock.desc':
    '启动后需通过 Touch ID（macOS）或本地 PIN 才将密码/私钥解密到内存。默认关闭。仅证明本机用户在场，不是云端 Passkey。',
  'settings.system.vaultUnlock.setupHint': '设置 4–12 位数字 PIN，作为平台认证不可用时的备用方式。',
  ...
  'vault.unlock.misconfigured': '已启用解锁，但没有可用的 PIN 或平台认证。请在 设置 → 系统 中关闭。',
