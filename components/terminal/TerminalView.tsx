              ...
              {inWorkspace && onToggleBroadcast && (
                onUpdateBroadcastConfig ? (
                  <BroadcastToolbarControl
                    enabled={Boolean(isBroadcastEnabled)}
                    onToggle={onToggleBroadcast}
                    config={broadcastConfig ?? DEFAULT_BROADCAST_CONFIG}
                    onUpdateConfig={onUpdateBroadcastConfig}
                    sessionOptions={broadcastSessionOptions ?? []}
                    sourceSessionId={sessionId}
                    sourceGroupPath={host?.group ?? ''}
                    allSessionRefs={broadcastAllSessionRefs ?? []}
                  />
                ) : (
                  <Tooltip>
                    <TooltipTrigger asChild>
        ...
        >
          ...
          {zmodem.active && (
            <div className="absolute bottom-4 right-4 z-[25] pointer-events-auto">
              ...
              />
      <ConfirmDialog
        open={Boolean(dangerousPasteDialog)}
        title={t('terminal.paste.dangerous.title')}
        message={
          dangerousPasteDialog
            ? t('terminal.paste.dangerous.message', {
                command: dangerousPasteDialog.sampleLine || '',
                pattern: dangerousPasteDialog.matchedPattern || '',
              })
            : undefined
        }
        confirmLabel={t('terminal.paste.dangerous.confirm')}
        destructive
        onOpenChange={handleDangerousPasteDialogOpenChange}
        onConfirm={handleDangerousPasteConfirm}
      />
            </div>
          )}
